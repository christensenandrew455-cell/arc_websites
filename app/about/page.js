...
export default function AboutPage() {
  return (
    <main className="min-h-screen bg-white text-slate-950">
      <SiteHeader />
      <section className={`border-b border-orange-200 px-4 py-10 text-center sm:px-6 sm:py-20 ${config.theme.heroBg}`}>
        <div className="mx-auto max-w-4xl">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-orange-600">About ARK Client Center</p>
          <h1 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">A simpler way to keep customer opportunities organized.</h1>
          <p className="mx-auto mt-4 max-w-3xl text-base leading-7 text-slate-700 sm:mt-5 sm:text-lg sm:leading-8">ARK Client Center connects receptionist intake, leads, conversations, and employees without forcing business owners to give out their private numbers.</p>
        </div>
      </section>

      <div className="mx-auto max-w-5xl space-y-4 px-3 py-6 sm:space-y-5 sm:px-6 sm:py-14">
        {sections.map((section) => (
          <section key={section.title} className={`${config.theme.sectionBg} p-4 sm:p-8`}>
