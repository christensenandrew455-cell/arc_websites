...
export default function SupportPage() {
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (!response.ok || !data.success) throw new Error(data.error || "Your request could not be submitted.");
      setForm(initialForm);
      setScreenshot(null);
      setStatus("success");
    ...
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <section className="border-b border-orange-200 bg-gradient-to-b from-orange-50 to-white px-4 py-14 text-center sm:px-6 sm:py-20">
        <div className="mx-auto max-w-4xl">
          ...
          <h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">How can ARK help?</h1>
          <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-slate-600">Ask about the product, get account help, make a privacy request, or report a messaging concern.</p>
        </div>
      </section>
        ...
        <div className="rounded-3xl border border-orange-200 bg-white p-6 shadow-lg sm:p-9">
          {status === "success" ? (
            <div className="py-16 text-center">
              ...
              <p className="mx-auto mt-3 max-w-xl leading-7 text-slate-600">ARK will use the contact information you provided if a response or more information is needed.</p>
              <button type="button" onClick={() => setStatus("idle")} className="mt-6 rounded-xl bg-orange-500 px-5 py-3 text-sm font-black text-white hover:bg-orange-600">Send Another Request</button>
            </div>
          ) : (
