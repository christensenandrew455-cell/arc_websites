...
function StoreOption({ title, description, href, label }) {
  return (
    <article className="rounded-3xl border border-orange-200 bg-white p-6 shadow-sm sm:p-8">
    ...
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <section className="border-b border-orange-200 bg-gradient-to-b from-orange-50 to-white px-4 py-14 text-center sm:px-6 sm:py-20">
        <div className="mx-auto max-w-4xl">
          ...
          <h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Open ARK Client Center.</h1>
          <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-slate-600">Use the web app now. Mobile store downloads can be added here when their listings are ready.</p>
        </div>
      </section>

      <div className="mx-auto max-w-5xl space-y-5 px-4 py-10 sm:px-6 sm:py-14">
        <section className="rounded-3xl border border-orange-300 bg-orange-50 p-6 shadow-sm sm:p-8">
        ...
        <section id="live-demo" className="rounded-3xl border border-orange-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          {demoPhoneHref ? (
            ...
            <span className="mt-6 inline-flex rounded-xl border border-slate-300 bg-slate-50 px-5 py-3 text-sm font-black text-slate-500">Live demo number coming soon</span>
          )}
        </section>
